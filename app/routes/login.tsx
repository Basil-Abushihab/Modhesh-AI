...
export default function LoginPage() {
  ...
  // Handle language direction + locale changes
  useEffect(() => {
  ...
  return (
    <DefaultLayout>
      <div className="flex flex-col items-center justify-center min-h-screen bg-bolt-elements-bg-depth-1">
        ...
        >
          {/* Title */}
          <h2 className="text-2xl font-bold text-center text-bolt-elements-textPrimary">{t('auth.welcome_back')}</h2>

          {/* Subtitle */}
          <p className="text-center text-bolt-elements-textSecondary text-sm mb-2">
            {t('auth.log_in_to')} <span className="font-semibold text-blue-500">{t('auth.modhesh_ai')}</span>
          </p>
